...
while(cap.isOpened()):
    ...
    if_face, img_face = check_if_face(frame)
    if_motion, img_face_motion = check_if_motion(img_face)

    current_time = datetime.datetime.now().strftime('%y-%m-%d %H:%M:%S')
    payload = {"ID": "B09611007", "Name": "Berlin", "Time": current_time
                ,"motion":str(if_motion),"face":str(if_face)}

    img_display = cv2.resize(img_face_motion,(540,360))
    img_display = cv2.putText(img_display,current_time,(10,346),cv2.FONT_HERSHEY_DUPLEX, 0.8, (0, 0, 0), 1, cv2.LINE_AA)
