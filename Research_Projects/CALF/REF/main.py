...
def get_opt():
    opt = parse_opts() # 可以解析command line裡的--命令(定義於"opts.py")
    ...
    opt.arch = '{}-{}'.format(opt.svm, opt.model_depth)
    opt.begin_epoch = 1
...
def resume_model(resume_path, arch, model, time_feature=False, n_classes=2):  # 給inference 使用
    print('loading checkpoint {} model'.format(resume_path))
    checkpoint = torch.load(resume_path, map_location='cpu')
    assert arch == checkpoint['arch']
    if time_feature:
        class MyModel(nn.Module):   # time featuring 訓練model
            def __init__(self):
                super(MyModel, self).__init__()
                self.cnn = model
                self.cnn.fc = nn.Linear(self.cnn.fc.in_features, n_classes)  # 這裡需要更改 在modle.py的load pretrain model中可以參考
                self.fc1 = nn.Linear(n_classes, 7)

            def forward(self, x, t):
                x1 = self.cnn(x)
                x2 = t
                x3 = torch.cat((x1, x2), 1)  # 把visual 跟 time 沿著一維(0=行,1=列)合在一起
                out = self.fc1(x3)
                return out
        model = MyModel()
    else:
        ...
        class featureModel(nn.Module):
            def __init__(self):
                ...
                self.cnn = model  # in feature=512     # 這裡就是把原本的model繼承到自定義的模型
                self.cnn.fc = nn.Linear(model.fc.in_features, n_classes)
...
def get_train_utils(opt, model_parameters):
    ...
    normalize = get_normalize_method(opt.mean, opt.std, opt.no_mean_norm,
                                     opt.no_std_norm)
    if not opt.no_hflip:   # 水平翻轉
        spatial_transform.append(RandomHorizontalFlip())
    if opt.colorjitter:    # 隨機調整圖片的亮度、對比、飽和度和色調
        spatial_transform.append(ColorJitter())
    spatial_transform.append(ToTensor())
    if opt.input_type == 'flow':  # or rgb
...
def main_worker(index, opt):
    random.seed(opt.manual_seed)   # 產生亂數種子
    ...
    if opt.pretrain_path:
        ...
        model = load_pretrained_model(model, opt.pretrain_path, opt.svm,  # 要注意這邊新增一個time feature的option
                                      opt.n_finetune_classes, opt.time)
    if opt.resume_path is not None:   # 給inference使用的
        model = resume_model(opt.resume_path, opt.arch, model, opt.time, n_classes=opt.n_classes)    # 所以這邊也需要多新增一個time feature的option
    ...
    criterion = CrossEntropyLoss().to(opt.device)   # 在這邊要設定loss function
    # criterion = MultiMarginLoss().to(opt.device)   # SVM
    if not opt.no_train:
    ...
    prev_val_loss = None
    for i in range(opt.begin_epoch, opt.n_epochs + 1):  # 開始訓練囉
        if not opt.no_train:
            if opt.distributed:
                train_sampler.set_epoch(i)
            current_lr = get_lr(optimizer)
            train_loss = train_epoch(i, train_loader, model, criterion, optimizer,    # 訓練的東西(from training (重要))
                                     opt.device, current_lr, train_logger,
                                     train_batch_logger, tb_wr
                                     iter, opt.distributed, opt.time)


            if i % opt.checkpoint == 0 and opt.is_master_node: # 在每一個checkpoint存當前資料於result_path中
        ...
        if not opt.no_train and opt.lr_scheduler == 'multistep':   # scheduler: learning rate調度器 e.g. StepLR、MultiStepLR、ReduceLROnPlateau 
            scheduler.step()
...
if __name__ == '__main__':
    opt = get_opt() # options: 模型訓練時需要的參數
