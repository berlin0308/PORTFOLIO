...
class Flood {
    ...
    public int village(int villages, List<int[]> road) {
        ...
        for(int[] EachEdge: road){
            DiGraph.addEdge(new DirectedEdge(EachEdge[0], EachEdge[1], EachEdge[2]));
        }
        DijkstraSP SP = new DijkstraSP(DiGraph, 0);

        return SP.findLastFlooded();
    }
...
class DijkstraSP {
    ...
    public int findLastFlooded(){
        double max = 0;
        int max_village=0;
        for(int v=1;v<distTo.length;v++){
            if(distTo[v]>max && distTo[v]!=Double.POSITIVE_INFINITY){
                max = distTo[v];
                max_village = v;
            }
        }
