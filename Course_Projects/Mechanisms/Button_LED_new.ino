...
String MODE = "STOP"; // MODE: STOP, FORWARD, RIGHT, LEFT, BACKWARD, JAWOPEN, JAWCLOSE 
...
String update_MODE(){
    if(checkIfSTOP()==true){
        ...
        if(count_btn_HIGH()>=2 || count_btn_HIGH()==0){
            ...
            if(digitalRead(btn_Jaw_Open)==HIGH){
                return "JAWOPEN";
            }
            if(digitalRead(btn_Jaw_Close)==HIGH){
                return "JAWCLOSE";
            }
        }
    }
...
int count_btn_HIGH(){
    int count = digitalRead(btn_F)+digitalRead(btn_R)+digitalRead(btn_L)+digitalRead(btn_B)+digitalRead(btn_Jaw_Open)+digitalRead(btn_Jaw_Close);
