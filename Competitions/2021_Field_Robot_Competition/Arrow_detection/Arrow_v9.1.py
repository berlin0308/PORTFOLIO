...
# def write_Serial(direction):
    # try:
        # print("Start writing serial...")
        # inputStr = str(direction) + str('e')
        # if inputStr == 'Re':
...
while cap.isOpened():  # Capture frame-by-frame
    ...
    if ret == True:
        ...
        if Distance < 120:
            ...
            if Distance < 60 and countLess > 2:
                print("Distance < 60 !!!")       
                ArrowDirResult = find_Arrow_Dir_Result(countDirR,countDirL,countDirN,totalDirs)
                print("\nStart to turn",ArrowDirResult) 
                write_Serial(1,1,1,ArrowDirResult,0,0,1,1,1,1)
                countLess = 0
                
        else:
            print("No Arrow")
            write_Serial(1,1,1,0,40,40,3,3,3,3)
