...
"""
"""
配合Arduino/Arduino_main_ed1.ino
放置箭頭在車前大於150cm
"""
...
def Area2Distance(area):
    dist = 2495.5*pow((float(area)),-0.461)  
    return int(dist)  
...
def Restart():
    countImage,totalDir,countDirR,countDirL,countDirN,countLess = 0, 0, 0, 0, 0, 0
    return countImage,totalDir,countDirR,countDirL,countDirN,countLess
...
while cap.isOpened():  # Capture frame-by-frame
    ...
    if ret == True:
        if RESTART == True:
            countImage,totalDir,countDirR,countDirL,countDirN,countLess = Restart()
            RESTART = False
            print("\n\n\nvalues have been restarted\n\n\n")
        ...
        if Distance < 120 and Distance < 60: # observe arrow direction -- 120cm to 60cm            
            totalDirs += 1
            if Dir == 'R':
                countDirR += 1
            if Dir == 'L':
            ...
            if totalDirs%10 == 0:
                TURN_SIG = True
                print("count:",totalDirs)
                print("\nR:%0.2f \nL:%0.2f \nN:%0.2f \n"%(countDirR/totalDirs,countDirL/totalDirs,countDirN/totalDirs))   
                currentResult = find_Arrow_Dir_Result(countDirR,countDirL,countDirN,totalDirs) 
        ...
        """ if Distance < 60 indeed"""
        if Distance < 60 and countLess > 3:   # delay : (frame time)*10
            print("Distance < 60 !!!")    
        ...
        """ normal mode, find lane"""
        if DRIFT == False:
            """ pwmL, pwmR, ledA, ledB, ledC, ledD = find_lane(frame) 
                    write_Serial(1,1,1,0,pwmL,pwmR,ledA,ledB,ledC,ledD) """
            pwmL = 60
            pwmR = 60
            if TURN_SIG == False:
                write_Serial(1,1,1,0,pwmL,pwmR,3,3,0,0)  # forward and no direction, green light on 
                
            if TURN_SIG == True:
                if currentResult == 'L' :
                    if ( totalDirs // 10 ) % 2 == 0
                        write_Serial(1,1,1,0,pwmL,pwmR,6,0,0,0)
                    if ( totalDirs // 10 ) % 2 == 1
                        write_Serial(1,1,1,0,pwmL,pwmR,0,0,0,0)
                if currentResult == 'R' :
                    if ( totalDirs // 10 ) % 2 == 0
                        write_Serial(1,1,1,0,pwmL,pwmR,0,6,0,0)
                    if ( totalDirs // 10 ) % 2 == 1
                        write_Serial(1,1,1,0,pwmL,pwmR,0,0,0,0)
        ...
        if DRIFT == True:           
            if ArrowDirResult == 'L':
                write_Serial(1,1,1,1,0,0,4,4,0,0)  # turn left, blue light on
            elif ArrowDirResult == 'R':
                write_Serial(1,1,1,2,0,0,7,7,0,0)  # turn right, megenta light on
            
            DRIFT = False
            RESTART = True
            
            write_Serial(1,1,0,0,0,0,2,2,0,0)  # red light on
            time.sleep(2)
