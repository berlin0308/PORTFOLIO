...
#SKIP = True  # skip to 'FORWARD' state - finding Box, start the ultra 
SKIP = False
...
TOMATO_threshold = 3600
LEMON_threshold = 4500
AVOCADO_threshold = 6000
...
def find_Box(frame):
    ...
    Smax_green_phase1 = 255
    Vmax_green_phase1 = 255
    boxgreen_area, BOXGREEN, frame_boxgreen = find_boxgreen(crop,70,Hmin_green_phase1,Smin_green_phase1,Vmin_green_phase1,Hmax_green_phase1,Vmax_green_phase1,Vmax_green_phase1)
...
def drop_Fruit():
    write_Serial(1,2,0,'d',0,0,1,1,2,2)
...
while cap.isOpened():  # Capture frame-by-frame
    ...
    if ret == True:
        ...
        if state == 'SLOWDOWN_1':
            ...
            ledA,ledB,ledC,ledD = MOTION_STATE_LEDs(state)
            write_Serial(1,2,1,0,pwmL,pwmR,ledA,ledB,ledC,ledD)
            if Interval < Critical_Interval:
                Fruit = find_Fruit(frame)
            else:
                Fruit = "AVOCADO"
                print("No fruit -> AVOCADO")
        ...
        if state == 'SLOWDOWN_2':
            ...
            if Box != "void":
                if Box == Fruit_to_Box(Fruit):
                    ...
                    if Box == "BOXGREEN":
                        write_Serial(1,2,0,0,0,0,3,3,3,3)
                    if Box == "BOXBLUE":
                        write_Serial(1,2,0,0,0,0,4,4,4,4)
        ...
        if state == 'FORWARD_3':
            ...
            pwmL,pwmR = MOTION_STATE_PWM(state)
            ledA,ledB,ledC,ledD = MOTION_STATE_LEDs(state)
            write_Serial(1,2,1,'f',60,60,2,2,2,2)
